...
async def main():
    async with SessionFactory() as db:
        ...
        try:
            ...
            upd = await routers.update_agent(
                agent["id"], AgentUpdate(status="paused", name="Test Bot 2"), user, db
            )
            ...
            up = UploadFile(filename="func-test.txt", file=io.BytesIO(
            ...
            conv2 = await routers.update_conversation(
                conv["id"], ConversationUpdate(status="resolved"), user, db
            )
            check("update_conversation (resolve)", conv2["status"] == "resolved")

            # ---- Chat (SSE, real Gemini) ----
            resp = await routers.chat(
                conv["id"], MessageIn(role="user", text="What is the refund policy?"), user, db
            )
            frames = []
            async for chunk in resp.body_iterator:
                frames.append(chunk)
            body = "".join(frames)
            check("chat streamed", '"type": "sources"' in body and "refund" in body.lower(), body.replace("\n", " ")[:160])
            ...
            check("dashboard perAgent", len(dash["perAgent"]) == 1
            ...
            check("download web link -> 302 source",
            ...
            loc = str(getattr(file_resp, "headers", {}).get("location", "")) \
                if getattr(file_resp, "status_code", 0) == 302 else ""
            check("download file -> 302 signed URL",
            ...
            check("document_preview file (chunks)",
            ...
            try:
                ...
                check("document_preview cross-account -> 404",
                      e.status_code == 404 and "different account" in e.detail)
            ...
            check("download-url file signed url",
                  "backblazeb2.com" in furl["url"] and doc["storageKey"].split("/")[-1] in furl["url"])

            halt = await routers.update_conversation(
                conv["id"], ConversationUpdate(status="halted"), user, db
            )
            check("update_conversation (halt)", halt["status"] == "halted")
            reopen = await routers.update_conversation(
                conv["id"], ConversationUpdate(status="active"), user, db
            )
            ...
            conv_b = await routers.create_conversation(
                ConversationCreate(visitor="DelMe", agent_id=None), user, db
            )
            await routers.add_message(
                conv_b["id"], MessageIn(role="user", text="temporary thread"), user, db
            )
            await routers.delete_conversation(conv_b["id"], user, db)
            gone = (await db.execute(
                select(func.count()).select_from(Message)
                .where(Message.conversation_id == conv_b["id"])
            )).scalar_one()
            ...
            check("settings_status",
                  stat["db_configured"] is True and stat["b2_enabled"] is not None)
            ...
            chunk_ids = (await db.execute(
                select(DocumentChunk.id).where(DocumentChunk.user_id == user.id)
            )).scalars().all()
