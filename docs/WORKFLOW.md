...
- **Phase 5 — in progress, 5A + 5B done**: **Privacy + Terms pages** (`/legal/privacy`, `/legal/terms`) linked in the landing footer + added to sitemap; landing CTAs ("Start Free Trial", "Get Started") now route to `/signup`. **Settings** is a real page — Profile card (Clerk), Service Status card (`GET /api/settings/status`: DB + B2 flags), and a **Danger Zone** with typed-confirm **Delete workspace** (`DELETE /api/me` purges all data + B2 originals, idempotent — a fresh `users` row upserts on next login). **Dashboard** shows a data-driven onboarding checklist card for new workspaces. Next: 5C conversation delete + chat rate limit + logging, 5D launch ops + final regression.
