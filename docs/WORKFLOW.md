...
- **Phase 4 — in progress, milestones 4A + 4B + 4C done**: chat sources carry `docId`; document `download` (302) / `download-url` (JSON) / `preview` (~2 KB) endpoints; `delete_document` B2 cleanup; `PATCH` status incl `halted`/`reopen`; **Chat page** (`/chat`) with history, live SSE, retry + follow-ups, sources view/download, resolve/halt; **Knowledge Base rows** now Open/Download + Delete; **Dashboard** adds Agent Performance table (convs/msgs/resolved/latency) + 7-day CSS trend bars. Remaining: 4D CI/CD (.github/workflows + render.yaml B2 env).
